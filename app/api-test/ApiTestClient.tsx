 ...
 * @returns Promise resolving to a `RequestResult` (always — failures don't throw).
 */
const runProbe = async (opts: {
  ...
  try {
    ...
    if (typeof body.responseTime === 'number') result.serverTime = body.responseTime;
 ...
 * @returns JSX pill with label + value (+ optional hover tooltip).
 */
const Stat = ({
  label,
  value,
 ...
 *
 * @returns JSX of the benchmark dashboard.
 */
export default function ApiTestClient(): JSX.Element {
  ...
  const startRun = useCallback(async (): Promise<void> => {
     ...
     *
     * @returns `false` when the run is exhausted or aborted; `true` after a probe.
     */
    const dispatchNext = async (): Promise<boolean> => {
      if (ctrl.signal.aborted) return false;
      const i = nextIndex++;
      if (i >= count) return false;
      const r = await runProbe({ index: i, preset, marker, cached, signal: ctrl.signal });
      collected.push(r);
  ...
  const successResults = useMemo(() => orderedResults.filter(r => r.success), [orderedResults]);
  const stats = useMemo(() => computeStats(successResults.map(r => r.time)), [successResults]);
  const serverStats = useMemo(
    () =>
      computeStats(
        successResults.map(r => r.serverTime).filter((v): v is number => typeof v === 'number')
      ),
    [successResults]
  );
  ...
  const progress = count > 0 ? Math.min(100, (orderedResults.length / count) * 100) : 0;
  ...
  }, [preset, marker, cached, mode, concurrency, count, orderedResults, stats, runs]);

  return (
    <div className="mx-auto max-w-323 px-4 py-8 text-paper">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold">OneEntry API Benchmark</h1>
        <p className="mt-1 text-sm text-paper/60">Dev dashboard for probing OneEntry endpoints.</p>
      </header>
      ...
      <section className="grid gap-4 rounded-panel border border-paper/10 bg-white/5 p-4 md:grid-cols-2">
        ...
        <div>
          ...
          <input
            ...
            placeholder={preset === 'products' ? '(not used for products)' : 'e.g. home_web'}
        ...
        <div>
          ...
          <div className="mt-2 flex gap-2">
            <button
              ...
              className={`rounded-card border px-3 py-1.5 text-sm ${
                !cached ? 'border-brand bg-brand text-white' : 'border-paper/15 text-paper'
              }`}
            >
              Raw SDK
            </button>
          ...
          <p className="mt-2 text-xs text-paper/50">
            Raw = direct <code className="text-paper/70">getApi()</code> call. Cached =
            project&apos;s <code className="text-paper/70">unstable_cache + cache()</code> wrapper.
          </p>
        </div>
              ...
              {COUNT_OPTIONS.map(n => (
                <button
                  ...
                  className={`rounded-card border px-3 py-1.5 text-sm tabular-nums ${
                    count === n ? 'border-brand bg-brand text-white' : 'border-paper/15 text-paper'
                  }`}
                >
                  {n}
                </button>
            ...
            <table className="w-full min-w-225 text-left text-xs tabular-nums">
              ...
              <tbody>
                {tableRows.map(r => {
                  ...
                  const network =
                    r.success && typeof r.serverTime === 'number' ? r.time - r.serverTime : null;
                  return (
                    <tr key={r.index} className="border-t border-paper/5 hover:bg-paper/5">
                      <td className="px-2 py-1 text-right text-paper/50">{r.index + 1}</td>
                      ...
                      <td className="px-2 py-1 text-right">{r.success ? formatMs(r.time) : '—'}</td>
                      <td className="px-2 py-1 text-right text-paper/70">
                        {typeof r.serverTime === 'number' ? formatMs(r.serverTime) : '—'}
                      </td>
        ...
        <section className="mt-6 rounded-panel border border-paper/10 bg-white/5 p-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-180 text-left text-xs tabular-nums">
              ...
              <tbody>
                {runs.map(r => {
                  const s = computeStats(r.results.filter(x => x.success).map(x => x.time));
                  const ok = r.results.filter(x => x.success).length;
                  return (
                    <tr key={r.id} className="border-t border-paper/5">
                      <td className="px-2 py-1 text-paper/70">
                        {new Date(r.startedAt).toLocaleTimeString()}
                      </td>
                      <td className="px-2 py-1">{r.preset}</td>
                      <td className="px-2 py-1 text-paper/70">{r.marker || '—'}</td>
                      <td className="px-2 py-1">
                        {r.mode === 'parallel' ? `par×${r.concurrency}` : 'seq'}
                      </td>
                      <td className="px-2 py-1">{r.cached ? 'cached' : 'raw'}</td>
                      <td className="px-2 py-1">{r.count}</td>
                      <td className="px-2 py-1">
                        <span className="text-emerald-300">{ok}</span>
                        <span className="text-paper/40"> / </span>
                        <span className={ok < r.count ? 'text-red-300' : 'text-paper/40'}>
                          {r.count - ok}
                        </span>
                        {r.aborted && (
                      ...
                      <td className="px-2 py-1">{s ? formatMs(s.mean) : '—'}</td>
