 ...
 * @returns JSX `<svg>` of the bar chart.
 */
export const BarChart = ({ results }: { results: RequestResult[] }): JSX.Element => {
  const finite = results.filter(r => r.success && Number.isFinite(r.time) && r.time >= 0);
  ...
  return (
    ...
    >
      ...
      {results.map((r, i) => {
        ...
        const h = r.success && r.time > 0 ? Math.max(2, (r.time / yScale) * innerH) : 6;
 ...
 * @returns JSX `<svg>` of the histogram.
 */
export const Histogram = ({ results }: { results: RequestResult[] }): JSX.Element => {
  const times = results
  ...
  for (const t of times) {
    const idx = Math.min(HIST_BIN_COUNT - 1, Math.floor((t / max) * HIST_BIN_COUNT));
  ...
  return (
    ...
    >
      ...
      {bins.map((count, i) => {
        ...
        return (
          <rect key={i} x={x} y={y} width={w} height={h} fill="#22c55e" opacity={0.85}>
 ...
 *
 * @returns JSX inline legend (color swatch + label per bucket).
 */
export const Legend = (): JSX.Element => {
  const buckets: Bucket[] = ['fast', 'ok', 'medium', 'slow', 'verySlow', 'failed'];
  return (
