 ...
 * @returns Aggregate `Stats` or `null` when the input is empty.
 */
 ...
 * @returns Formatted string like `"123.45 ms"`; `"—"` when the input is non-finite.
 */
export const formatMs = (v: number): string => (Number.isFinite(v) ? `${v.toFixed(2)} ms` : '—');

/**
 * formatBytes — pretty-prints a byte count using KB / MB units when appropriate.
 *
 * @param   {number} v - Byte count.
 * @returns Formatted string with the largest unit that keeps the number ≥ 1.
 */
