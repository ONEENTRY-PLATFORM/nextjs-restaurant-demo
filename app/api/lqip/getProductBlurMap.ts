 ...
 *
 * Runs `getLqipPreview` in parallel for every product that has an `images` entry. The helper reuses
 * the existing in-process LRU in `getLqipPreview`, so warm calls return immediately.
 *
