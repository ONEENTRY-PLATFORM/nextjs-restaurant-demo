 ...
 * @returns `true` when the value matches one of the supported presets.
 */
 ...
 * @param   {string} lang   - Locale code (e.g. `en_US`).
 * @param   {boolean} cached - When true, use the project's `unstable_cache`-wrapped fetcher.
 * @returns Promise resolving to a minimal payload signature `{ ok, size }`.
 */
const runRequest = async (
  ...
  if (preset === 'page') {
    ...
    if (cached) {
      const { isError, page } = await getPageByUrl(m);
  ...
  if (preset === 'menu') {
    ...
    if (cached) {
      const { isError, menu } = await getMenuByMarker(m);
  ...
  if (preset === 'block') {
    if (!marker) throw new Error('Block preset requires a `marker` query param');
    if (cached) {
  ...
  const data = await getApi().Products.getProducts([], lang, { offset: 0, limit: 12 });
  return { ok: !!data, size: data ? JSON.stringify(data).length : 0 };
};
 ...
 * @returns Promise resolving to JSON `{ success, responseTime, preset, marker, cached, size }` or `{ success: false, error }`.
 */
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const lang = sp.get('lang') ?? LanguageEnum.en;
