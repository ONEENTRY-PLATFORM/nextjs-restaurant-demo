...
describe('handleApiError', () => {
  ...
  it('maps an IError-shaped value to ApiError with the same statusCode/message', () => {
    const result = handleApiError('h', { statusCode: 404, message: 'Not found' });
...
describe('formatErrorMessage', () => {
  it.each([
    ...
    expect(formatErrorMessage({ statusCode, message: 'ignored' })).toBe(expected);
  });

  it('falls back to IError.message for an unmapped status code', () => {
    expect(formatErrorMessage({ statusCode: 418, message: 'I am a teapot' })).toBe('I am a teapot');
  });

  it('falls back to defaultMessage when IError.message is empty and status is unmapped', () => {
    expect(formatErrorMessage({ statusCode: 418, message: '' }, 'fallback')).toBe('fallback');
  });
...
describe('useApiErrorHandler', () => {
  ...
  beforeEach(() => {
    ...
    toastSpy = jest.spyOn(toast, 'error').mockImplementation(() => 'toast-id' as never);
    jest.spyOn(console, 'log').mockImplementation(() => {});
  });
  afterEach(() => {
    jest.restoreAllMocks();
