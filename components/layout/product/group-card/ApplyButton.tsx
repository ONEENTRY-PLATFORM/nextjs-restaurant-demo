...
const ApplyButton = ({ product }: { product: IProductsEntity }): JSX.Element => {
  ...
  return !productInCart || !inCart ? (
    <button
      ...
      className="rounded-card border border-brand text-brand px-4 py-1.5 mt-auto text-sm font-bold hover:text-paper active:bg-brand-soft-active active:text-white disabled:border-ink disabled:text-ink"
    >
      {t('apply_text', 'Apply')}
    </button>
  ) : (
