...
// нотификационных тогглов. Каждая группа маркеров скрывается по своей
// причине:
//   - `repeat_password` — UI-only поле подтверждения пароля,
//     используемое формой `sign_up`. В профиле смены пароля нет
//     отдельной кнопки "повторить пароль", поэтому второй input
//     не нужен.
//   - `email_notifications`, `email_notification_reg` — настройки
//     рассылок. По вёрстке в Personal они не появляются (под них есть
//     отдельная секция/экран настроек уведомлений), поэтому скрываем,
//     чтобы не путать с полями профиля.
//   - `user_address`, `user_flat`, `user_floor` — адресные поля, у
//     которых в попапе есть отдельная секция Address (со списком
//     сохранённых адресов и формой добавления нового). Если бы они
//     попали в Personal, пользователь видел бы их дважды — в Personal
//     как "плоский" набор инпутов и в Address как структурированный
//     список. Скрываем здесь, рендерим только в секции Address.
