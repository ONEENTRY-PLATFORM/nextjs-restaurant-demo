...
/**
 * Fields rendered in the first 2-column row.
 * @private
 */
 ...
 * textarea for preferences, primary submit + secondary "Sign up" / "Book a table".
 *
 * Fields are driven by OneEntry form `reservation` metadata — the component
 * auto-renders whatever is configured, and gracefully degrades if expected
 * markers are missing.
  ...
  return (
    ...
    >
      ...
      {ROW_PAIRS.map(([left, right]) => {
        ...
        return (
          <div key={left + right} className="flex justify-between gap-[15px]">
            {leftAttr ? (
              <Field attr={leftAttr} values={values} onChange={onChange} />
            ) : (
              <div />
            )}
            {rightAttr ? (
              <Field attr={rightAttr} values={values} onChange={onChange} />
            ) : (
              <div />
            )}
          </div>
      ...
      {attrs
        ...
        .map((a) => (
          <Field key={a.marker} attr={a} values={values} onChange={onChange} />
        ))}
 ...
 */
const Field = ({ attr, values, onChange }: FieldProps): JSX.Element => {
