...
import formFieldsReducer, { addField } from '@/app/store/reducers/FormFieldsSlice';

/**
 * initial — fresh `formFieldsSlice` state via the reducer's `@@INIT` path.
 *
 * @returns Initial form-fields-slice state.
 */
...
describe('FormFieldsSlice — addField', () => {
  ...
  it('replaces an existing field by marker (last write wins)', () => {
    ...
    state = formFieldsReducer(state, addField({ email: { value: 'new@x.io', valid: true } }));
    expect(state.fields.email).toEqual({ value: 'new@x.io', valid: true });
  });

  it('preserves other fields when adding a new one', () => {
    let state = formFieldsReducer(initial(), addField({ email: { value: 'a@b.cd', valid: true } }));
    state = formFieldsReducer(state, addField({ phone: { value: '+1', valid: false } }));
